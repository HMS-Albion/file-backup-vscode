 ...
 * `.bak` under targetPath by copying, verifying with SHA256, then deleting.
 *
 * The path travels in FB_ROOT rather than on the command line, so names with
 * spaces or shell metacharacters cannot break the invocation.
 */
export function stripBakSuffix(scriptPath: string, targetPath: string): Promise<StripResult> {
  return new Promise<StripResult>((resolve, reject) => {
    const child = spawn(
      "powershell.exe",
      ["-NoProfile", "-NonInteractive", "-ExecutionPolicy", "Bypass", "-File", scriptPath],
      {
        env: { ...process.env, FB_ROOT: targetPath },
        windowsHide: true,
      }
    );
