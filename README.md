...
4. **删除源文件**：只删"已确认 `.bak` 副本逐字节一致"的那些；不一致就保留源文件并在提示里报。
5. **PowerShell 复原名字**：同一个 `strip-bak.ps1`，后缀列表由 `FB_SUFFIXES` 传入（`.bak`、`.c`、`.c1`），每个后缀最多剥一次，于是 `notes.txt.c.bak → notes.txt`、`legacy.c.c1.bak → legacy.c`。
...
- 右键**文件夹**：数据最终在 `<目录名>.bak/` 里，名字已全部复原，连原本就叫 `.bak` / `.bak.1` 的文件也在其中并保住原名；`<目录名>/` 被搬空（只剩空目录与目录树）。目录本身不改名也不删除。
